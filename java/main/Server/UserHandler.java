...
public class UserHandler extends Thread {
	...
	private void deleteUser() throws Exception {
		// logout user and remove from all online and friends lists
		...
		for(Map.Entry<String, UserModel> users : server.getUserMap().entrySet()) {
			...
			if(user.getFriendRequests().contains(this.username)) {
				user.removeFriendRequest(this.username);
			}
		}
		...
		// delete .txt file and blocked users file
		String regFile = this.username + ".txt";
		String blockFile = this.username + "_blocked.txt";
		Files.delete(Paths.get("Data/", regFile));
		Files.delete(Paths.get("Data/", blockFile));
	}
	...
	private void blockUser(String blockedUserString) throws Exception {
		...
		if (blockedUser == null) {
			...
			msg = "Success! You have blocked " + blockedUserString
					+ ". You can unblock by sending them a friend request.";
		}
